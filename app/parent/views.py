...
from . import parent
from .forms import RoleForm, EmployeeAssignForm, SchoolForm, StudentAddForm, MessageForm, QueryForm, ReplyForm
from .. import db
from ..models import Department, Forward, Message, School, Member, Student, Teacher, Grade, Course, Query, Parent, Mappingquery, Staff
...
#parent asking QueryForm
@parent.route('/queries', methods=['GET', 'POST'])
@login_required
def write_query():
    """
    writing a query
    ...
    if form.validate_on_submit():
        ...
        student = Member.query.filter_by(id=form.studentid.data.id).first()
        student1 = Student.query.filter_by(studentid=student.id).first()
        #grade = Grade.query.filter_by(id=student.grade)
        if (form.tag.data == 'subject'):
            #member = Member.query.filter_by(username=current_user.username)
            member1 = Mappingquery.query.filter_by(tag=str(student1.grade)).first()
            teacher = Teacher.query.filter_by(teacherid=member1.memberid).first()
            if (teacher.onleave == False):
                #member = Member.query.filter_by(username=current_user.username)
                query = Query(senderid=member.id, receiverid=member1.memberid, tag=form.tag.data,
                ...
                query = Query(senderid=member.id, receiverid=original.replacerid, tag=form.tag.data,
                                              description=form.description.data, schoolid=student.schoolid)
    ...
    return render_template('parent/queries/parent_query.html', form=form, title='Queries')

@parent.route('/reply_message/<int:id>', methods=['GET', 'POST'])
@login_required
def reply_message(id):
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('home.parent_dashboard'))
    return render_template('parent/messages/reply_message.html', form=form, title='reply')


@parent.route('/messages', methods=['GET', 'POST'])
@login_required
def post_message():
    """
    writing a message
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('home.parent_dashboard'))
    return render_template('parent/messages/message_post.html', form=form, title='Messages')


#student asking QueryForm
@parent.route('/addstudent/', methods=['GET', 'POST'])
@login_required
def add_student():
    """
    writing a query
    ...
    if form.validate_on_submit():
         #School.query.filter_by(schoolname=form.schoolname.data).first() is not None:
        if Student.query.filter_by(studentid=form.studentid.data).first() is not None:
        #member = Member.query.filter_by(username=current_user.username)
            student = Student.query.filter_by(studentid=form.studentid.data).first()
            parent = Parent(parentid=current_user.id,
            ...
            return redirect(url_for('home.parent_dashboard'))
    return render_template('parent/addstudent.html', form=form, title='Add Student')
